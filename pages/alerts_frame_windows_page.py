...
class AlertsPage(BasePage):
    ...
    def check_alert_appear_5_sec(self):
        self.element_is_visible(
            self.locators.ALERT_AFTER_FIVE_SEC_BUTTON).click()
        time.sleep(5)
        alert = self.switch_to_alert()
        return alert.text
...
class ModalDialogsPage(BasePage):
    ...
    def check_small_modal(self):
        ...
        title = self.element_is_visible(self.locators.SMALL_MODAL_TITLE).text
        self.element_is_visible(self.locators.SMALL_MODAL_CLOSE_BUTTON).click()
        return title

    def check_large_modal(self):
        self.element_is_visible(self.locators.LARGE_MODAL_BUTTON).click()
        title = self.element_is_visible(self.locators.LARGE_MODAL_TITLE).text
